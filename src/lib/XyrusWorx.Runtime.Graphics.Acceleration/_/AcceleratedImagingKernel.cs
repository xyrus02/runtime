...
{
	[PublicAPI]
	public class AcceleratedImagingKernel2 : AcceleratedExecutionContext, IImagingKernel
	{
		...
		public AcceleratedImagingKernel([NotNull] AccelerationDevice provider, [NotNull] AcceleratedKernel bytecode) : base(provider, bytecode)
		{
		...
		private HardwareRenderTarget CreateRenderTargetBuffer(DeviceContext context, int arrayWidth, int arrayHeight)
		{
			Execution.ExecuteOnUiThread(() =>
			{
			...
			var buffer = new HardwareRenderTarget(Provider, mSwapChain);
		...
		private IDataStream<Vector4<byte>> ComputeUsingPsProfile(Device device, DeviceContext context, int arrayWidth, int arrayHeight)
		{
			using (var output = CreateRenderTargetBuffer(context, arrayWidth, arrayHeight))
			{
				...
				var sourceDescription = output.Texture.Description;
				...
				context.CopyResource(output.Texture, emitBuffer);
		...
		class ShaderResourceList : AcceleratedKernelResourceList<IStructuredBuffer>
		{
			...
			protected override void SetElement(IStructuredBuffer item, int index)
			{
				var rv = new[]
				{
					item.CastTo<HardwareTexture>()?.ResourceView,
